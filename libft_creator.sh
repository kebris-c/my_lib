...
LOCAL_LIB="$HOME/Desktop/libs/srcs/"
LOCAL_HEADER="$HOME/Desktop/libs/includes/"
...
if compgen -G $HOME/Desktop/libs/includes/*.h > /dev/null; then
    rm -f "$HEADER_DIR"*.h
    cp "$LOCAL_HEADER"ft_*.c $HEADER_DIR
    echo "✅ Header files copied to '$HEADER_DIR'"
else
    echo "⚠️  No header files found in '', skipping copy."
fi
...
select TYPE in "Shared (.so)" "Static (.a)"; do
    case $REPLY in
        1)
            ...
            $CC $CFLAGS -shared -fPIC $LOCAL_LIB -I$LOCAL_HEADER/*.h -o $LIB_DIR/$LIB_NAME.so
            if [ $? -eq 0 ]; then
        ...
        2)
            ...
            $CC $CFLAGS $LOCAL_LIB -I$LOCAL_HEADER/*.h -o $LIB_NAME.o
            if [ $? -ne 0 ]; then
